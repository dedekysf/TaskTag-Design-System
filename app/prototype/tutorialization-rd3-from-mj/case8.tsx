    ...
    <View style={{ height: 28, alignItems: 'center', justifyContent: 'flex-end', paddingBottom: 9 }}>
      <View
        style={{
          ...
          height: 5,
          borderRadius: 5,
          backgroundColor: light ? 'rgba(255,255,255,0.5)' : '#000',
        }}
      />
    </View>
  );
  ...
  return (
    <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 12, paddingHorizontal: 16, paddingVertical: 8 } as any}>
      ...
      <View style={{ flex: 1, gap: 8 } as any}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 } as any}>
          <Text style={{ fontSize: 16, fontWeight: '600', color: TTTheme.colors.darkMagenta }}>
...
export function Case8Screen({ onComplete }: Case8ScreenProps = {}) {
  ...
  const [showComposeTooltip, setShowComposeTooltip] = useState(true);
  ...
  const handleSelectTask = (task: TaggableTask) => {
    ...
    setPhase('compose');
    setShowComposeTooltip(true);
    setMessageText('');
    messageTextRef.current = '';
  };

  const scheduleComposeTooltipFade = (text: string) => {
    if (!text.trim() || !showComposeTooltip) return;

    if (composeTooltipTimerRef.current) clearTimeout(composeTooltipTimerRef.current);
    composeTooltipTimerRef.current = setTimeout(() => {
      Animated.timing(composeTooltipAnim, {
        toValue: 0,
        duration: 300,
        useNativeDriver: true,
      }).start(() => setShowComposeTooltip(false));
    }, 2000);
  };
  ...
  const handleChangeText = (text: string) => {
    ...
    setMessageText(text);
    scheduleComposeTooltipFade(text);
  };
    ...
    <Box flex={1} backgroundColor="white">
      ...
      {phase === 'coach' && (
        <OnboardingTooltip
          ...
          style={{ top: 572, left: 46, zIndex: 62 }}
          arrowEdge="bottom"
          arrowSide="left"
          arrowInset={40}
