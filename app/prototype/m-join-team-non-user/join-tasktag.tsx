...
import { BatteryFull, ChevronRight, Hammer, KeyRound, Link, Lock, LogOut, PieChart, SignalHigh, Star, UserPlus, Users, WifiHigh, Zap } from 'lucide-react-native';
import { router } from 'expo-router';
import React from 'react';
import { Image as RNImage, Pressable, ScrollView } from 'react-native';
      ...
      <Box flex={1} width="100%" maxWidth={480} backgroundColor="grey02">
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 16, paddingBottom: 32 }}>
          ...
          >
            ...
            <Text variant="webLabelEmphasized" color="white" textAlign="center">James Hammer invited you</Text>
            <Text variant="webMetadataPrimary" color="white" textAlign="center" style={{ opacity: 0.7 }}>
              One step away from joining this team.
            </Text>
            <Pressable
              onPress={() => router.push('/prototype/m-join-team-non-user/join-tasktag-signup' as any)}
              style={({ pressed }) => [{
                width: '100%', paddingVertical: 12, borderRadius: 8,
                alignItems: 'center', backgroundColor: '#18A87D', marginTop: 4,
                opacity: pressed ? 0.85 : 1,
              }]}
            >
              <Text variant="webLabelEmphasized" style={{ color: '#fff' }}>Join This Team</Text>
            </Pressable>
          </Box>
...
function InfoRow({ label, value, valueColor }: { label: string; value: string; valueColor?: string }) {
  return (
    <Box
      ...
      paddingVertical="sm"
    >
      <Text variant="webLabelEmphasized">{label}</Text>
      {value ? (
        <Text variant="webSecondaryBody" style={{ color: valueColor ?? TTTheme.colors.mutedForeground }}>{value}</Text>
      ) : null}
    </Box>
  );
}
